...
const Navbar = () => {
    const [surat, setSurat] = useState([]); // ✅ Ubah dari {} ke []

    const getDataFromAPI = () => {
        fetch("https://equran.id/api/v2/surat")
            .then((res) => res.json())
            .then((data) => {
                setSurat(data.data); // ✅ Pastikan mengambil "data.data"
            })
            .catch((error) => {
    ...
    return (
        <div className="bg-dark text-white vh-100 p-3 overflow-auto" style={{ width: "250px" }}>
                            ...
                            {surat.map((surah) => (
                                <li key={surah.nomor} className="nav-item">
                                    <Link to={`/surat/${surah.nomor}`} className="nav-link text-white">
                                        <i className="bi bi-flower3"></i> {surah.nomor}. {surah.namaLatin}
                                    </Link>
                                </li>
                            ))}
                        </ul>
